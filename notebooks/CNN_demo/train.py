...
from pyimagesearch.ConvolutionalLayer import ConvolutionLayer
...
print("[INFO] using {}...".format(device))
...
numBufferSamples = len(trainData) - numTrainSamples - numValSamples
...
# initialize the ConvolutionLayer model
print("[INFO] initializing the Convolution model...")
model = ConvolutionLayer(numChannels=1, classes=len(trainData.dataset.classes)).to(
    device
)
...
print(
    classification_report(
        testData.targets.cpu().numpy(), np.array(preds), target_names=testData.classes
    )
)
